...
for i in range(num_routers - 1):
    G.add_edge(i, i + 1, weight=np.random.randint(1, 10))  # Pozitif ağırlıklı kenarlar
...
ani = FuncAnimation(fig, update, frames=np.arange(num_routers), init_func=init, interval=750)  # Her 0.6 saniyede bir güncelleme yap
...
def show_shortest_path(event):
    ...
    if len(selected_faculties) == 2:
        ...
        shortest_path = nx.dijkstra_path(G, index1, index2, weight='weight')  # Dijkstra algoritması burada kullanılıyor
        edges = list(zip(shortest_path[:-1], shortest_path[1:]))
        nx.draw_networkx_edges(G, pos, edgelist=edges, edge_color='red', width=3.0, ax=ax)
        plt.axis('off')
        ani.event_source.stop()  # Animasyonu durdur
        plt.draw()
